...
def tasks():
	...
	closed_tasks = [dict(name=row[0], due_date=row[1],
		priority=row[2], task_id=row[3]) for row in cur.fetchall()]
	g.db.close()
	#return render_template('task.html', form=AddTaskForm(request.form),open_tasks=open_tasks,closed_task=closed_task)
...
def new_task():
	...
	if not name or not date or not priority:
		flash("Todos los datos son requeridos, Intente de nuevo")
		return redirect(url_for('tasks'))
	else:
